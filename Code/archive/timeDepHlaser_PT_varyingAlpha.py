...
epsrel=10**(-5)
# duration (ps)
dur = 50 # can go up to current PT length
...
# is this correct?
#A = alpha
def gaussian_shape(t, area = 30.0, tau = 10.0, t_0 = t_0): #area was 1
...
t_plot = np.linspace(0,int(dur),int(dur)*10) #
...
file_name = 'Laser_alpha='+str(alpha)+'_T='+str(T)+'_dt='+str(dt)+\
...
# Read in PT
full_path_PT = 'C:/Users/sony/Desktop/Project/process_tensor/Om_1t_alpha=0.001_T=0_dt=0.05_dkmax=70_epsrel=1e-05_dur=100.0_PT.txt'
...
file_name_t = 'Laser_alpha='+str(alpha)+'_T='+str(T)+'_dt='+str(dt)+\
    '_dkmax='+str(dkmax)+'_epsrel='+str(epsrel)+'_dur='+str(dur)+'_Time.txt'
    
file_name_s_z = 'Laser_alpha='+str(alpha)+'_T='+str(T)+'_dt='+str(dt)+\
