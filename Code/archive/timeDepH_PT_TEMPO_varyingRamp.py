...
coeff = [0.0001,0.025,0.01,0.0075]
...
file_name_PT = 'Om_'+str(coeff[0])+'t_alpha='+str(alpha3)+'_T='+str(T)+'_dt='+str(dt)+\
    '_dkmax='+str(dkmax)+'_epsrel='+str(epsrel)+'_dur='+str(dur[0])+'_PT.txt'
...
# Plot
# to change scale: multiply by time dep omega?
plt.plot(t1, s_z1, label=r'TEMPO $\Omega(t)$={0}t'.format(coeff[0]))
# plt.plot(t2, s_z2, label=r'TEMPO $\Omega(t)$={0}t'.format(coeff[1]))
# plt.plot(t3, s_z3, label=r'TEMPO $\Omega(t)$={0}t'.format(coeff[2]))
# plt.plot(t4, s_z4, label=r'TEMPO $\Omega(t)$={0}t'.format(coeff[3]))
