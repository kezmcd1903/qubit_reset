...
# Spectral density Delta (like one mode)
J = 4*alpha*w*np.exp(-(w-J_shift)**2/10**(-2))

def J(x):
    return 3.67879*np.exp(-(x-J_shift)**2/10**(-2))


plt.plot(w,J(w),color='black')#.format(coeff) tau = 0.245
...
for i in range(len(Omega)):
    plt.axvline(x = Omega[i],color=c[i],linestyle='dashed',label=r"$\Omega_{0}={1}$".format(i,Omega[i]))
...
# For a different spectral density
correlations = oqupy.correlations.CustomSD(
                                j_function=J,
                                cutoff_type='hard',
                                cutoff=omega_cutoff,
                                temperature=temperature)
...
for i in range(len(Omega)):
    ...
    # Spectral density delta function
    J[i] = 4*alpha*Omega[i]*np.exp(-(Omega[i]-J_shift)**2/10**(-2))
